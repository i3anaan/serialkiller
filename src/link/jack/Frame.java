...
public class Frame {
	...
	public Frame() {
		for (int i = 0; i < PAYLOAD_UNIT_COUNT; i++) {
			if (this.units[i] == null) {
				this.units[i] = JackTheRipper.UNIT_IN_USE.getFiller();
			}
		}
	}

	/*
	 * public Frame() { for(int i=0;i<units.length;i++){ units[i] = new
	 * Unit(PureUnit.FLAG_FILLER_DATA); }
	...
	public Frame(BitSet2 data) {
		...
		if (JackTheRipper.UNIT_IN_USE instanceof PureUnit) {
			for (int i = 0; i < data.length() - 7 && i/8<this.PAYLOAD_UNIT_COUNT; i = i + 8) {
				this.units[i / 8] = new PureUnit(Bytes.fromBitSet(data, i));
			}
		} else if (JackTheRipper.UNIT_IN_USE instanceof HammingUnit) {
			for (int i = 0; i < data.length() - 7 && i/8<this.PAYLOAD_UNIT_COUNT; i = i + 8) {
				this.units[i / 8] = new HammingUnit(data.get(i, i+8),JackTheRipper.HC);
			}
		}
		// Fill empty spots with Filler data flags.
		for (int i = 0; i < PAYLOAD_UNIT_COUNT; i++) {
	...
	/**
	 * Returns only the data contained in this Frame. (Flags and stuffings are
	 * filtered out).
	 * 
	 * @return
	 */
	...
	/**
	 * Returns all the bits in this payload (Flags and stuffings are left in).
	 * 
	 * @return
	 */
	...
	public Frame getClone() {
		...
		Frame result = new Frame(newUnits);
		return result;
	}
